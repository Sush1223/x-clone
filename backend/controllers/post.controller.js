...
export const createPost = async (req, res) => {
  try {
    const { text } = req.body;
    let { img } = req.body;
    // converting to string because may req.user is not in a string
    ...
    if (img) {
      const uploadedRes = cloudinary.uploader.upload(img);
...
export const getAllPosts = async (req, res) => {
  // -1 gives the latest post at top
  try {
    // Yes, the populate method in Mongoose is specifically designed to work with references between collections. For populate to function correctly, the user field in the Post schema must be set up as a reference to the User collection
